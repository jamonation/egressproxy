...
func (s *HTTPProxy) Listen(addr string) error {
	s.Server = http.Server{
		Addr:    addr,
		Handler: http.HandlerFunc(s.egressproxyRequest),
	}
...
func (s *HTTPProxy) egressproxyRequest(w http.ResponseWriter, req *http.Request) {
	var err error
	var resp *http.Response

	if !s.checkAccess(req) {
...
func (s *HTTPProxy) handleConnect(w http.ResponseWriter, r *http.Request) {
	dstConn, err := net.DialTimeout("tcp", s.upstreamTLSProxy, 1*time.Second)
	if err != nil {
